...
/**
 * FE-3: ダッシュボードルート（src/app/page.tsx）のデータ取得連携テスト。
 *
 * page.tsx は Server Component として fetchSensorsGeoJson() で実データを取得し、
 * SensorMap へ渡す。バックエンドが応答しない場合はフォールバック（モック GeoJSON）
 * で描画する。fetchSensorsGeoJson と SensorMap のみをモックし、
...
describe("Home (page.tsx)", () => {
  it("取得した実データを SensorMap へ渡して描画する", async () => {
