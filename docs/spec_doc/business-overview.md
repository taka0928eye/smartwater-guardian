...
バックエンド起動時、hydrants.json 実在23消火栓が常に **severity_level=0（正常）** で
自動初期化される（コマンド不要）。ダッシュボードの「シード投入」ボタン（`POST
/api/v1/demo/seed-batch`）で23消火栓へ Lv0×11 / Lv1×8 / Lv2×3 / Lv3×1 を一括投入し、
「シードクリア」ボタン（`DELETE /api/v1/demo/clear`）で23件Lv0の初期状態に戻せる。
「防災シミュレーション」ボタン（`POST /api/v1/disaster/simulate`）は実在23消火栓の
