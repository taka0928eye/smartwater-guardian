...
> しうる（`list_alerts()` はセンサー単位で重複排除しない設計のため）。Level 3
> の並び順検証（`nth(0-2)` 等）はseedで severity=3 に上書きされる分が上位に
> ソートされるため影響しないが、`dashboard.spec.ts`「6. Level 0（正常）の表示
> 切替」・`alerts.spec.ts`「正常も表示」トグルで参照する `HYD-002`（Lv0）は
> 起動時レコードとシードレコードの2件が一致し、Playwright の厳格モード
> （複数要素へのマッチ）でテストが不安定化する可能性がある。E2E を実行する際は
> 事前に動作確認し、不安定な場合は `pages/DashboardPage.ts` の `alertRow()` に
> `.first()` を付与するか、`global-setup.ts` で別の未使用センサーIDを使う対応を
> 検討すること（本ドキュメントの更新時点では未検証・未修正）。
