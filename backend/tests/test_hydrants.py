...
def test_hydrants_json_contains_exactly_23_records():
    assert len(load_hydrants()) == 23


def test_hydrant_and_sensor_ids_are_unique():
    hydrants = load_hydrants()

    assert len({hydrant["hydrant_id"] for hydrant in hydrants}) == 23
    assert len({hydrant["sensor_id"] for hydrant in hydrants}) == 23
